...
/**
 * GooglePayPaymentCompletedEvent is fired after the actual payment is completed.
 * We are linking the Finix transaction to the newly created WooCommerce order.
 */
class GooglePayPaymentCompletedEvent extends Event {
	...
	public function process(): array {
		...
		$is_success     = sanitize_text_field( wp_unslash( $_POST['finix_google_pay_success'] ?? 'false' ) );
		$transaction_id = sanitize_text_field( wp_unslash( $_POST['finix_google_pay_transaction_id'] ?? '' ) );
		...
		if ( $is_success !== 'true' || empty( $transaction_id ) ) {
			...
			$this->gateway->log(
				'Finix Payment error: Could not validate payment. Google Pay returned empty transaction ID.',
				'error',
				[
		...
		$this->gateway->log( 'Finix Google Pay Payment came with transaction ID: ' . $transaction_id, 'debug' );
		...
		if ( empty( $transaction_details['response']->id ) ) {
			...
			$this->gateway->log( 'Finix Google Pay Payment error: Could not validate payment. Transaction details returned empty.', 'debug' );
		...
		$this->order->add_order_note(
			esc_html__( 'Finix Google Pay payment SUCCEEDED', 'finix-for-woocommerce' )
		);
