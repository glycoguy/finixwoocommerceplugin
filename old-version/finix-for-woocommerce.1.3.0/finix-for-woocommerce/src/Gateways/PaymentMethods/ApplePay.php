...
/**
 * Pay using Finix via Apple Pay.
 */
final class ApplePay extends FinixMethod {

	/**
	 * Payment method name defined by payment methods extending this class.
	 */
	protected $name = ApplePayGateway::SLUG;
	...
	public function initialize(): void {
		...
		$this->settings = get_option( 'woocommerce_finix_apple_pay_gateway_settings', [] );
	 ...
	 */
	public function get_payment_method_script_handles(): array { // phpcs:ignore Generic.Metrics.CyclomaticComplexity
		...
		// Check if the script is registered, and if not, register it.
		if ( ! wp_script_is( 'finix-apple-pay', 'registered' ) ) {
			// phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
			wp_register_script( 'finix-apple-pay', Assets::APPLE_SDK_JS_URL, [], null, true );

			$handles[] = 'finix-apple-pay';
		}
		...
		wp_localize_script(
			'finix_apple_pay_gateway-checkout-block',
			'finix_apple_pay_params',
			[
				'nonce'             => wp_create_nonce( 'get_secret_action' ),
				'merchant_identity' => sanitize_text_field( $merchant_info['response']->identity ?? '' ),
				'merchant_name'     => sanitize_text_field( $this->gateway->merchant_name ),
				...
				'button_style'      => esc_attr( $this->gateway->button_style ),
				'button_type'       => esc_attr( $this->gateway->button_type ),
		...
		wp_set_script_translations( 'finix_apple_pay_gateway-checkout-block', 'finix-for-woocommerce' );
	...
	private function get_common_payment_method_script_handles( array $handles ): array {
		...
		wp_register_script(
			'finix_apple_pay_gateway-checkout-block',
			Assets::url( 'js/block/build/apple-pay.js', false ),
			array_merge(
				( include FINIXWC_PLUGIN_DIR . 'assets/js/block/build/apple-pay.asset.php' ) ['dependencies'],
				is_admin() ? [] : [ 'finix-sdk', 'finix-apple-pay' ]
			),
			Assets::ver(),
			true
		);

		$handles[] = 'finix_apple_pay_gateway-checkout-block';
